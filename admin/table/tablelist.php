    ...
    <div class="flex flex-col w-full bg-pink-300 gap-4 p-2">
        ...
        echo '<hr/><h2 class="text-2xl">Create a New Table On <span class="font-bold uppercase text-blue-600">'
            . $databaseName . '  </span>              
                    ...
                    <form action="table.php?db=' . $databaseName . '" method="post" class="flex flex-col gap-2 xl:w-3/4 px-2 justify-center">
    ...
    if ($tablesNames) {
        ...
        foreach ($tablesNames as $tablesName) {
            echo '<li class="w-full h-14 flex justify-center">
               <form action="table.php?db=' . $databaseName . '&table=' . $tablesName . '&Branch_Name=" method="post" class="ring-1 flex p-2 w-full justify-between">
                   <input type="hidden" name="formType" value="deleteTable">
                   <a href="tableDetails.php?db=' . $databaseName . '&table=' . $tablesName . '&Branch_Name=" class="m-0 p-0  capitalize">' . $tablesName . '</a>
